...
- A datamodel document indexes to its declared paths and their types, at
  every nesting depth, through one total function.
```
...
- A change to what the document admits, or to what the read check accepts, is
  breaking for every consumer holding a document. Say what to change in the
  document.
